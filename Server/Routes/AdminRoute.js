...
router.put("/edit_user/:id", (req, res) => {
  ...
  db.query(sql, [email, password, id], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ Status: false, Error: "Failed to update the user." });
    }
...
router.get("/attendance", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ Status: false, Error: "Failed to fetch attendance data" });
  }
});
...
router.post("/add_payslips", (req, res) => {
  const { employee_id, date, amount, details, deduction, netPay } = req.body;

  // Assuming 'con' is your database connection
  const sql = `INSERT INTO payslips (employee_id, date, amount, details, deduction, net_pay) VALUES (?, ?, ?, ?, ?, ?)`;
  const values = [employee_id, date, amount, details, deduction, netPay];

  con.query(sql, values, (err, result) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ Status: false, Error: "Failed to add payslip" });
    }

    return res.status(200).json({ Status: true, Message: "Payslip added successfully" });
  });
});


router.get('/payslips', (req, res) => {
  const sql = "SELECT * FROM payslips";
  con.query(sql, (err, result) => {
      if (err) {
          return res.json({ Status: false, Error: "Query Error" });
      } else {
